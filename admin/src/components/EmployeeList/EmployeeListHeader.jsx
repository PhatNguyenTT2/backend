...
export const EmployeeListHeader = ({
  ...
  useEffect(() => {
    ...
    if (showActionsDropdown || showDepartmentFilter) {
  ...
  }, [showActionsDropdown, showDepartmentFilter]);

  const handleDepartmentSelect = (deptId) => {
    setSelectedDepartment(deptId);
    onFilterByDepartment && onFilterByDepartment(deptId);
    setShowDepartmentFilter(false);
  };

  const getSelectedDepartmentName = () => {
    if (selectedDepartment === 'all') return 'All Departments';
    if (selectedDepartment === 'none') return 'No Department';
    const dept = departments.find(d => d.id === selectedDepartment);
    return dept ? dept.departmentName : 'All Departments';
  };
        ...
        <div className="flex items-center gap-2 flex-1 ml-4">
          ...
          <div className="relative" ref={departmentDropdownRef}>
            <button
              ...
              className="h-[36px] px-3 bg-white border border-[#ced4da] rounded-lg text-[12px] font-['Poppins',sans-serif] text-[#212529] flex items-center gap-2 hover:bg-gray-50 transition-colors focus:outline-none focus:ring-2 focus:ring-emerald-500"
            >
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M13 2H1L5.8 7.46V11.5L8.2 13V7.46L13 2Z" stroke="#6B7280" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
              <span className="truncate max-w-[120px]">{getSelectedDepartmentName()}</span>
              <svg width="10" height="10" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg" className={`transition-transform ${showDepartmentFilter ? 'rotate-180' : ''}`}>
                <path d="M3 4.5L6 7.5L9 4.5" stroke="#212529" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </button>
            ...
            {showDepartmentFilter && (
              <div className="absolute left-0 mt-2 w-[200px] bg-white rounded-lg shadow-xl border border-gray-200 py-1 z-50 max-h-[300px] overflow-y-auto">
                <button
                  onClick={() => handleDepartmentSelect('all')}
                  className={`w-full px-4 py-2 text-left text-[12px] font-['Poppins',sans-serif] hover:bg-gray-50 transition-colors ${selectedDepartment === 'all' ? 'bg-emerald-50 text-emerald-600' : 'text-gray-700'
                    }`}
                >
                  All Departments
                </button>
                <button
                  onClick={() => handleDepartmentSelect('none')}
                  className={`w-full px-4 py-2 text-left text-[12px] font-['Poppins',sans-serif] hover:bg-gray-50 transition-colors ${selectedDepartment === 'none' ? 'bg-emerald-50 text-emerald-600' : 'text-gray-700'
                    }`}
                >
                  No Department
                </button>
                ...
                {departments.map((dept) => (
                  <button
                    ...
                    className={`w-full px-4 py-2 text-left text-[12px] font-['Poppins',sans-serif] hover:bg-gray-50 transition-colors ${selectedDepartment === dept.id ? 'bg-emerald-50 text-emerald-600' : 'text-gray-700'
                      }`}
                  >
                    {dept.departmentName}
                  </button>
